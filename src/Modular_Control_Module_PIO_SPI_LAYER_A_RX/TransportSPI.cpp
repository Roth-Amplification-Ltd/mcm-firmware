...
{
  // Drop all queued packets. Any in-flight packet already loaded in the PIO TX FIFO
  // will still shift out if the master clocks it, but the queue is emptied and IRQ
  // is immediately deasserted. The master should follow RESYNC by clocking a fresh
  // snapshot to restore a known-good state.
  _txHead = 0;
  _txTail = 0;
  _txCount = 0;
  _txByteIndex = 0;
